...
export default async function handler(req, res) {
  ...
  try {
    ...
    if (!originalNodes || originalNodes.length === 0) {
      res.status(400).json({ error: 'Original workflow nodes are required for optimization' });
      return;
    }

    if (!analysisResult || !analysisResult.weakPoints) {
      res.status(400).json({ error: 'Analysis results are required for optimization' });
      return;
    }
